...
def get_model(model_name: str):
    if model_name == 'logistic_regression':
        ...
        return DecisionTree(cfg_path='configs/classifiers/decision_tree.yml')
...
def experiment(dataset_name: str, algorithm: str, model_name: str, iteration: int, date: str, random_seed: int,
    ...
    dataset.train.to_csv(f'results/{algorithm}_{dataset_name}_{model_name}/{date}/train_{iteration}.csv')
    dataset.fair.to_csv(f'results/{algorithm}_{dataset_name}_{model_name}/{date}/fair_{iteration}.csv')
    ...
    perf.to_csv(f'results/{algorithm}_{dataset_name}_{model_name}/{date}/performance_{iteration}.csv',
                index=False)
    fairness.to_csv(f'results/{algorithm}_{dataset_name}_{model_name}/{date}/fairness_{iteration}.csv',
                    index=False)
...
if __name__ == "__main__":
    ...
    Parallel(n_jobs=-1)(delayed(experiment)(d_name, a, m_name, idx, date, seeds[idx], kfolds=kfolds, enc_type=encoding) for d_name, a, m_name, idx in all_options)

    if neptune:
        for d_name, a, m_name, idx in all_options:
            try:
                perf = pd.read_csv(f'results/{a}_{d_name}_{m_name}/{date}/performance_{idx}.csv')
                fairness = pd.read_csv(f'results/{a}_{d_name}_{m_name}/{date}/fairness_{idx}.csv')
